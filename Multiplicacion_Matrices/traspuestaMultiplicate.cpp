...
vector<vector<int>> multiplyMatrices(const vector<vector<int>>& A, const vector<vector<int>>& B) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < p; ++j) {
            for (int k = 0; k < m; ++k) {
                result[i][j] += A[i][k] * B[k][j]; // Mantener la implementación original
            }
        }
    }
...
int main() {
    ...
    for (const auto& fileA : squareFiles1) {
        for (const auto& fileB : squareFiles2) {
            ...
            cout<<matrixB.size()<<matrixB[0].size()<<endl;
            ...
            if (matrixA[0].size() != matrixB_t.size()) {
                cerr << "Dimensiones incompatibles para multiplicación: " << fileA << " y " << fileB << endl;
                continue;
            }
    ...
    for (const auto& fileA : rectangularFiles) {
        for (const auto& fileB : rectangularFiles) {
            if (fileA != fileB) {  // Evitar multiplicar una matriz por sí misma
                ...
                int rowsA = matrixA.size();
                int colsA = matrixA[0].size();
                int rowsB = matrixB_t.size();
                int colsB = matrixB_t[0].size();
    ...
    saveTimesToCSV(squareMultiplicationTimes, "square_multiplication_t_times.csv");
    saveTimesToCSV(rectangularMultiplicationTimes, "rectangular_multiplication_t_times.csv");
